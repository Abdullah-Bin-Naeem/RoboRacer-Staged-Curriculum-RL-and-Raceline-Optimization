...
def _default_raceline():
    """The line the follower would be driving, if this is running in the workspace.

    Overridable with $DIAG_PATH_CSV; None when the package is not on the ament
    index, which is fine -- the cross-track column is then simply blank.
    """
    try:
        from roboracer_stack.common.frames import DEFAULT_RACELINE
        return DEFAULT_RACELINE
    except Exception:
        return None
...
class Diag(Node):
    ...
    def __init__(self):
        ...
        for _csv in (os.environ.get('DIAG_PATH_CSV'), _default_raceline()):
