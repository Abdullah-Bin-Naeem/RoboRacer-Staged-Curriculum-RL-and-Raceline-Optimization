#!/usr/bin/env bash
# Run the two containers.   ./docker/run.sh sim [--headless] | racer
#
#     ./docker/run.sh sim --headless    simulator, no window, auto-connects
#     ./docker/run.sh sim               simulator with a window; hit Connect
#     ./docker/run.sh racer             our stack -- starts driving on its own
#
