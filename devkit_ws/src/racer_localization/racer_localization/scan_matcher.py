...
def rebase_repo_path(p, repo=None):
    """A path recorded inside the container -> the same file in this checkout.

    Logs and scan dumps carry absolute paths from wherever they were written
    (/root/Documents/roboracer in the image). Rebasing with relpath against
    frames.REPO produces nonsense when the two trees differ above the repo
    (it built ../../../../root/... and the replay tool died on it). Anchor on
    the repo-relative component instead, which is the same in both trees.
    """
    if not p or os.path.exists(p):
        return p
    repo = repo or os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                '..', '..', '..', '..'))
    parts = os.path.normpath(p).split(os.sep)
    for anchor in ('devkit_ws', 'raceline', 'experiments', 'maps'):
        if anchor in parts:
            cand = os.path.join(repo, *parts[parts.index(anchor):])
