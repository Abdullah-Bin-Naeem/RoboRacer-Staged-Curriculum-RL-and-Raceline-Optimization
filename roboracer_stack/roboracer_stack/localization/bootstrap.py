...
Three modes:
  ...
  mode:=spawn   (default)  Seed from the MEASURED spawn constant (spawn_x/y/yaw,
                           ...
                           and the IMU is a permitted sensor. The spawn is
                           repeatable to about 2 cm, so this is as exact as
                           truth. THE RACE DEFAULT.

  mode:=truth              Read /ips and /imu ONCE, seed the localizer with that
                           pose, verify it actually adopted it, release /ips,
                           and hand over. Also legal: the first lap is a warmup
                           and the timer starts after it, so a single read
                           before the car moves sits inside the permitted
                           window -- see roboracer_stack/common/restricted.py,
                           THE WARMUP WINDOW. The subscription is destroyed by
                           _release_truth() as soon as the seed resolves, so
                           the claim is enforced rather than asserted. Kept for
                           a track whose spawn has not been measured yet.

  mode:=global             No initial pose at all. Search the map at the IMU's
                           heading, creep forward on lidar alone, and wait for
                           the estimate to collapse. Reads no ground truth
                           either, but it is slower and less reliable -- one
                           stretch of corridor looks much like another, so the
                           car has to reach a corner before the ambiguity
                           breaks.

Every mode latches /localization_ready when done, which the follower waits on
instead of a fixed timer.

IT ALSO OWNS RECOVERY
---------------------
After the handover this node keeps watching the encoders, the IMU and the scan
for the signature of a wall contact, which the simulator answers by teleporting
the car back to a checkpoint. When it sees one it holds the follower, seeds the
localizer with the checkpoint behind the last trusted pose (common/frames
CHECKPOINTS), confirms that seed the same way as the first one, and releases the
car. The heading always comes from the IMU, which is absolute and legal.

WHY THIS IS A HANDSHAKE AND NOT A PUBLISH
-----------------------------------------
A localizer does not accept an initial pose until it is running. localization_v2
publishes map->odom only once it has processed its first scan, and anything sent
to /initialpose before that is dropped. The publisher here is VOLATILE, so there
is no late-joiner redelivery either: the message is simply gone.

Every node in the launch starts at the same instant, so whether the seed lands
would otherwise be a race between the sensors coming up and the localizer coming
up. When the seed was lost, the localizer stayed on its fallback prior and this
node declared /localization_ready anyway, handing the follower a pose that had
been wrong since startup.

So the sequence is:

    1. wait for the localizer to come up -- the appearance of map->odom
       (ready_check:=tf), or a lifecycle node reaching `active`
    2. publish the seed to /initialpose
    3. nudge the localizer into running an update, since the car is stationary
       and some filters only resample once they believe they have moved
...
---------------------------
Everything localizer-shaped is a parameter rather than a branch, because this
handshake was written to serve three different localizers. This branch races
one, and v2.launch.py sets them: ready_check:=tf (wait for map->odom to appear,
since a plain node has no lifecycle state to poll), the estimate on
/localization_v2/pose, and verification through TF.

THE ESTIMATE IS CONFIRMED ON TF, NOT ON THE POSE TOPIC
------------------------------------------------------
verify_via_tf reads map->roboracer_1, which is the localizer's correction
COMPOSED with dead reckoning -- the transform pure_pursuit actually consumes.
Confirming the seed against it checks the pose the car will really drive on,
rather than a topic the follower never reads.

The seed assumes map coordinates match the simulator's world frame. They do
here: the map was built with scan matching disabled, so its correction against
the world frame is identity.
"""
...
from roboracer_stack.common import frames, restricted
...
class LocalizationBootstrap(Node):
    ...
    def __init__(self):
        ...
        # Defaults from common/frames; v2.launch.py passes initial_x/y/yaw
        ...
        p('spawn_x', float(frames.SPAWN_X))
        p('spawn_y', float(frames.SPAWN_Y))
        p('spawn_yaw', float(frames.SPAWN_YAW))
        ...
        p('estimate_topic', '/amcl_pose')     # v2.launch.py: /localization_v2/pose
        ...
        # Empty string disables; localization_v2 offers both.
        ...
        if self.mode in ('truth', 'spawn') or self.recover:
        ...
        # TF is needed to notice the localizer coming up (ready_check=tf) and to
        ...
        # instead, since a forced relocalization republishes the estimate on
        # the very next scan anyway.
    ...
    def _ready_yet(self, now):
        ...
        lifecycle   a nav2 lifecycle node; poll get_state until ACTIVE.
        tf          localization_v2 is a plain node with no state to poll. It
                    publishes map->odom only after processing its first scan,
                    which is also when it starts accepting /initialpose, so the
                    transform existing IS the handshake. THIS BRANCH.
        """
    ...
    def _seed(self):
        ...
        if self.attempts == 1 and self.mode == 'truth':
            # Only in truth mode: in spawn mode the seed IS the constant, so
    ...
    def _report_spawn(self, x, y, yaw):
        ...
        truth mode only, and it exists to keep the SPAWN_* constant honest --
        because on this branch that constant is the RACE DEFAULT, not a
        fallback. A run seeded from /ips is the one chance to check it, so the
        measurement is printed whenever the two disagree.
        ...
        if gap <= 0.10:
            self.get_logger().info(
                ...
                'of common/frames.py SPAWN_*, so bootstrap_mode:=spawn is sound. '
                'Nothing to change.')
            return
        self.get_logger().warn(
            f'MEASURED SPAWN is {gap:.3f} m / {math.degrees(dyaw):.1f} deg from '
            'common/frames.py SPAWN_*, which is what bootstrap_mode:=spawn seeds '
            'from. This run is seeded from /ips and therefore fine; the RACE '
            'DEFAULT is what is wrong.\n'
            f'    Paste into roboracer_stack/common/frames.py:\n'
            f"        SPAWN_X = '{x:.3f}'\n"
            f"        SPAWN_Y = '{y:.3f}'\n"
            f"        SPAWN_YAW = '{yaw:.4f}'")
    ...
    def _record(self, x, y, yaw):
        ...
        if self.src_pos is not None and self.src_quat is not None:
            self.est_err = math.hypot(x - self.src_pos[0], y - self.src_pos[1])
            self.est_dyaw = abs(wrap(yaw - yaw_from_quat_xyzw(self.src_quat)))
    ...
    def _tick(self):
        ...
        if not self.localizer_up:
            self._ready_yet(now)
            if not self.localizer_up and self.scan is None:
                # THE CLOCK DOES NOT START UNTIL THE SENSORS DO. The container
                # is started BEFORE the operator presses Connect in the
                # simulator, so there is no lidar -- and therefore no map->odom,
                # which this localizer publishes only after its first scan --
                # until they do. Counting that wait against localizer_wait_s
                # parked the follower for the whole race because nobody pressed
                # a button within 30 s (seen on the first container start with
                # no simulator attached). Waiting on the bridge is the normal
                # case, not a fault.
                self.t0 = now
            if not self.localizer_up and now - self.t0 > self.localizer_wait:
                if self.ready_check == 'tf':
                    self._fail(
                        ...
                        'and is it receiving scans? The localizer publishes this '
    ...
    def _tick_seeded(self, now):
        """spawn and truth modes, and every recovery seed: seed, confirm the
        localizer adopted it, hand over."""
        if self.src_pos is None or self.src_quat is None:
            return                      # still waiting for the first /imu (and /ips)
        ...
        if fresh:
            ...
            if gap <= self.tol_m and dyaw <= self.tol_yaw and scan_ok and self._recover_ok():
                ...
                self._finish(
                    ...
                    'reckoning only.')
                return
            if (self.mode == 'spawn' and self.rec_state != 'seed' and gap > self.tol_m
                    and scan_ok and self._converged()):
                # The constant is measured, not sacred. If the filter settles
                # somewhere else, has converged, and the scan says that pose is
                # right against the map, then the constant has drifted and the
                # filter is correct -- accept where it settled rather than park
                # the car on a stale literal. The scan check is what makes this
                # safe; without it this branch would accept any confident error.
                self._finish(
                    f'{scan_text}. seed ACCEPTED OFF THE SPAWN CONSTANT on attempt '
                    f'{self.attempts}: the filter settled {gap:.2f} m / '
                    f'{math.degrees(dyaw):.1f} deg from it (pos std {self.pos_std:.3f} m). '
                    'Re-measure the spawn if this repeats.')
                return
    ...
    def _tick_global(self, now):
        if not self.global_srv:
            # Only reachable with global_service:='' -- a localizer with no
            # map-wide search. The one this branch ships has one.
            self._finish(
                'this localizer has NO global search, so mode:=global leaves it '
                'on its fallback prior, UNVERIFIED: if that is far from the true '
                'spawn the offset is frozen for the run. Use '
                'bootstrap_mode:=spawn instead -- the measured spawn constant, '
                'no restricted topic, and confirmed before the car moves.')
            return
    ...
    def _recovery_prior(self):
        ...
        yaw = yaw_from_quat_xyzw(self.src_quat) if self.src_quat else (self.last_good[2] if self.last_good else 0.0)
        lg = self.last_good
        if lg is not None:
            best = None
    ...
    def _tick_recover(self, now):
        ...
        if self.rec_state == 'flagged':
            ...
            self.src_pos, self.src_quat = (x, y), (0.0, 0.0, math.sin(yaw / 2.0), math.cos(yaw / 2.0))
        ...
        if self.rec_state == 'seed':
            ...
            self._send(0.0, 0.0)
            self._tick_seeded(now)            # seed -> confirm -> _finish/_fail, intercepted above
            return
    ...
    def _recover_global(self, message):
        ...
        if self.src_pos is not None and self.last_good is not None:
            self.get_logger().warn(
                f'UNMATCHED RESET near ({self.last_good[0]:+.2f}, {self.last_good[1]:+.2f}): if the car '
                'is at a checkpoint not in frames.CHECKPOINTS, add it from this run\'s ground truth')
    ...
    def _report(self):
        ...
        if not self.localizer_up:
            if self.scan is None:
                self.get_logger().info(
                    f'waiting for the simulator: no {NS}/lidar yet, so the '
                    'localizer has nothing to process. The bridge is listening '
                    'on 4567; press Connect. Nothing is timing out.')
                return
        ...
        if self.mode != 'global' and self.seeded_at is None:
            where = '/ips' if self.mode == 'truth' else 'the spawn constant'
            self.get_logger().info(
                f'waiting for the seed source: {where}={"ok" if self.src_pos else "--"} '
                f'imu={"ok" if self.src_quat else "--"}')
            return
        if self.mode != 'global':
            source = (f'TF {self.map_frame} -> {self.base_frame}'
                      if self.verify_via_tf else self.est_topic)
