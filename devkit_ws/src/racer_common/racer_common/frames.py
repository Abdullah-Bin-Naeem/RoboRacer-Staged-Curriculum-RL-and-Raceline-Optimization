...
TRACKS = {
    ...
    'iros2026': {
        # IROS 2026 practice scene (autodrive_roboracer_sim:2026-iros-practice).
        # Mapped 2026-09-16: 241 x 901 px at 0.025 m, origin (0.004, -16.8);
        # ~44 m lap, corridor ~1.37 m. One 17 m straight, two hairpins round
        # the middle wall's ends, two chevrons with r 0.8-1.9 m corners.
        # maps/iros2026/track_solid.pgm seals the LiDAR-visible gap in the
        # middle wall (x 1.53, y -3.1..-4.2) and two chevron-tip slivers;
        # optimize_raceline.py picks it up automatically. Never give it to AMCL.
        #
        # Spawn: measured on this scene (frames.py of the iros_compete_usman
        # branch, 0.818/3.158/-90 deg) -- on the long straight, heading -y.
        # The scenes share a spawn point, so this matches Porto/ICRA within
        # 2 cm; re-read /ips right after a reset if the bootstrap disagrees.
        'spawn': ('0.818', '3.158', '-1.5707'),
        # Ladder not climbed on the car yet. Start at the bottom rung and
        # promote here as each rung runs clean; place margin zones only from
        # analyze_run.py's corner bias and wall contacts, never by guess.
        # raceline_tum_iqp.csv (tph iqp_handler, kappa_bound 1.5, 0.20 m
        ...
        #
        # rl_mt_max_a7.0b_edit2.csv (the hand-edited rl_mt_max line, profile to
        # 9.51 m/s) was the default briefly on 2026-09-18 and is NOT: against
        # run mtb15's zero resets it logged 10 (run_edit_1) and 5 (run_edit_2)
