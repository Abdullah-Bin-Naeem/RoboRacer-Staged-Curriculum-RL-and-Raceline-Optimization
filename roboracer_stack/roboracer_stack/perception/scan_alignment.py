...
def _distance_field(img, res):
    ...
    scipy is imported here rather than at module scope because the submission
    image does not ship it -- an import at the top would make this file
    unloadable in the container, and this is a diagnostic, not part of the race
    stack. `pip install scipy` if you want to run it there.
    """
    from scipy import ndimage
    return ndimage.distance_transform_edt(img != 0) * res
...
class Check(Node):
    ...
    def __init__(self, want, map_base=DEFAULT_MAP):
        ...
        self.dist = _distance_field(img, self.res)
...
def main():
    ...
    if '/' not in map_base:
        map_base = os.path.join(MAPS_DIR, map_base)
