...
| icra 24 | hand-edited line raceline_a7.0zv_hard_l6.0_corners_h_edit4.csv (line_editor), tcp_nodelay ON (its first run) | 0, 15 respawns | -- | Two findings, separated by the log. (1) The LINE: planned lateral demand 17.8 m/s^2 at s 9.3 and 20 in the run-up to the first contact at s 22.8, car 5.12 where the line said 3.46; a hand-shaped profile has no rung cap, every solver line is <= 7.0 by construction. It hits regardless of the shim. (2) The SHIM: the loop rate was unchanged (18.9 Hz, as the probe said) but the follower's measured COMMAND DELAY slid 0.164 -> 0.056 s, already 0.088 before the first hit: the bridge's reply reaches the sim within the same frame instead of 1-2 frames later. Every follower constant was tuned at 0.175 and the lookahead scaling floors at 0.7 of it; a 60 ms loop has never been driven. tcp_nodelay back to OFF by default; test it on run 23's line before anything else. Also: first launch started with the car moving (not reset), and AMCL rejected every re-seed as 'malformed' -- a latent bug in the re-seed path, race-irrelevant (race starts from rest, first seed confirms) but to fix |
| loop rate, fix | tools/sim_rate_probe.py and the real bridge on loopback, Linux simulator only, same session (2026-09-12) | | 18.6 -> 77.3 Hz | Root cause found and fixed; the 2026-09-11 reading that the simulator's frame period is the limit was wrong (HUD 144 fps, cycle 50 ms = 7 frames). The simulator's ~13 KB telemetry is one sub-MSS segment on loopback (MTU 65536): Nagle holds it until our ACK and Linux delays that ACK, ~50 ms a cycle. The shim's read-side QUICKACK re-arm was undone by our own reply (Linux re-enters delayed-ACK mode on a send right after a receive); tools/libnodelay.so now re-arms after every send syscall too. Probe: no options 19.3, Python-level re-arm after emit 19.4 (no effect), shim preloaded 101.6, `lo` MTU 1500 alone 107. Bridge lidar topic, 25 s each: tcp_nodelay:=false 18.6 (median 52.7 ms), :=true 77.3 (median 12.8, max 25). Same simulator replied to by a Windows laptop over a direct LAN cable: 63.6 Hz, quantized by its 15.6 ms tick; that laptop (7th-gen i5, no GPU) renders the sim at 7.5 fps, so the organizer's two-machine route is moot here. tcp_nodelay stays OFF by default until the follower is re-validated at the fast loop (run 24) |
